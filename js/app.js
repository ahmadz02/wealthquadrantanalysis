...
function injectPeriodControls() {
  ...
  const revCtrl = document.createElement('div');
  revCtrl.id = 'revisionControls';
  revCtrl.className = 'revision-controls';
  revCtrl.innerHTML = `<button class="period-btn" id="revPrev" onclick="changeRevision(-1)" title="Previous revision">‹</button><select id="revisionSelect" onchange="setRevisionFromSelect()"><option value="0">Original Data</option><option value="1">Revision 1</option><option value="2">Revision 2</option><option value="3">Revision 3</option></select><button class="period-btn" id="revNext" onclick="changeRevision(1)" title="Next revision">›</button><div class="dup-wrap"><button id="duplicateBtn" class="hdr-btn dup-btn" onclick="toggleDupMenu(event)" disabled>Duplicate Data ▾</button><div id="dupMenu" class="dup-menu hidden"></div></div>`;
  const pillNav = topbar.querySelector('.pill-nav');
  topbar.insertBefore(ctrl, pillNav);
  topbar.insertBefore(revCtrl, pillNav);
  syncPeriodControls();
...
function noteKey(key) { return `wq-note-${currentYear}-${currentMonth}-r${currentRevision}-${key}`; }
...
async function loadCurrentPeriod() {
  ...
  syncRevisionControls();
  clearDynamicRows();
  const d = WQStorage.getMonthData(currentYear, currentMonth, currentRevision, defaultOnlineData);
...
async function saveData() {
  ...
  try { await WQStorage.saveMonthData(currentYear, currentMonth, currentRevision, collectData()); }
...
function renderAnalysis() {
  ...
  const revLabel = currentRevision === 0 ? 'Original Data' : `Revision ${currentRevision}`;
  R('a-date').textContent = `${MONTHS[currentMonth]} ${currentYear} — ${revLabel}`;
...
/* ── REVISION CONTROLS ── */
const REV_NAMES = ['Original Data', 'Revision 1', 'Revision 2', 'Revision 3'];
...
function syncRevisionControls() {
  ...
  if (sel)     sel.value      = String(currentRevision);
  if (btn)     btn.disabled   = currentRevision === 0;
  if (prevBtn) prevBtn.disabled = currentRevision === 0;
  if (nextBtn) nextBtn.disabled = currentRevision === 3;
  updateDupMenu();
}

function changeRevision(delta) {
  const next = currentRevision + delta;
  if (next < 0 || next > 3) return;
  currentRevision = next;
  loadCurrentPeriod();
}

function setRevisionFromSelect() {
  currentRevision = parseInt(document.getElementById('revisionSelect')?.value || '0', 10);
  loadCurrentPeriod();
}

function updateDupMenu() {
  const menu = document.getElementById('dupMenu');
  if (!menu) return;
  let html = '';
  for (let i = 0; i < currentRevision; i++) {
    html += `<button class="dup-opt" onclick="duplicateFrom(${i})">${i === 0 ? 'Copy from Original Data' : 'Copy from Revision ' + i}</button>`;
  }
  html += `<button class="dup-opt dup-opt-manual" onclick="duplicateManually()">Start Fresh (Manually Insert)</button>`;
  menu.innerHTML = html;
}

function toggleDupMenu(e) {
  e?.stopPropagation();
  ...
  if (isHidden) {
    setTimeout(() => document.addEventListener('click', closeDupMenu, { once: true }), 0);
  }
}

function closeDupMenu() {
  document.getElementById('dupMenu')?.classList.add('hidden');
}

function duplicateFrom(srcRevision) {
  closeDupMenu();
  const existing = WQStorage.getMonthData(currentYear, currentMonth, currentRevision, () => null);
  const hasData  = existing && (existing.salary > 0 || (existing.allow||[]).length > 0 || (existing.mtg||[]).length > 0 || (existing.cash||[]).length > 0);
  if (hasData && !confirm(`This will overwrite ${REV_NAMES[currentRevision]} data. Continue?`)) return;
  const srcData = WQStorage.getMonthData(currentYear, currentMonth, srcRevision, defaultOnlineData);
  clearDynamicRows();
  loadDataIntoForm(srcData);
  recalc();
  showToast(`Copied from ${REV_NAMES[srcRevision]}`);
}

function duplicateManually() {
  closeDupMenu();
  const existing = WQStorage.getMonthData(currentYear, currentMonth, currentRevision, () => null);
  const hasData  = existing && (existing.salary > 0 || (existing.allow||[]).length > 0 || (existing.mtg||[]).length > 0);
  if (hasData && !confirm(`Clear ${REV_NAMES[currentRevision]} and start with empty data?`)) return;
  clearDynamicRows();
  loadDataIntoForm(defaultOnlineData());
  recalc();
  showToast(`${REV_NAMES[currentRevision]} cleared — enter data manually`);
}

function showToast(msg, duration=2500) {
  let t = document.getElementById('wq-toast');
  if (!t) {
...
function labelAnalysisBlocks() {
  const blocks = document.querySelectorAll('#a-content > .grp-block');
  const ids = ['grp-overview', 'grp-budget', 'grp-networth', 'grp-health', 'grp-actionplan'];
  blocks.forEach((b, i) => { if (ids[i]) b.id = ids[i]; });
}

function applyRoleVisibility() {
  const role = window.WQAuth?.getProfile?.()?.role;
  const premiumOnly = role === 'premium';
  ['grp-budget', 'grp-networth', 'grp-health', 'grp-actionplan'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.display = premiumOnly ? 'none' : '';
  });
}

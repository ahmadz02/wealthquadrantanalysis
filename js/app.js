...
function getPlanningSuitePageHtml() {
  return `
<div class="page planning-suite-page" id="page-planning-suite">
  <div class="planning-suite-shell">
    <div class="planning-suite-head">
      <div>
        <div class="planning-suite-eyebrow">Planning Suite</div>
        <div class="planning-suite-title" id="planningSuiteTitle">Risk &amp; Takaful Planning</div>
        <div class="planning-suite-sub" id="planningSuiteSub">Selected module</div>
      </div>
    </div>
    <div class="planning-suite-tabs-wrap">
      <div class="planning-suite-tabs" id="planningSuiteTabs" aria-label="Planning Suite modules"></div>
    </div>
    <div class="planning-suite-module" id="planningSuiteModule"></div>
  </div>
</div>`;
}
...
function mountOnlineApp() {
  ...
  shell.querySelector('#page-planning-suite')?.remove();
  shell.insertAdjacentHTML('beforeend', WQ_ONLINE_HTML);
  if (window.WQPersonalData?.renderPage) {
    shell.insertAdjacentHTML('beforeend', WQPersonalData.renderPage());
  }
  window.WQObjectives?.mountIntoPersonalPage?.();
  shell.insertAdjacentHTML('beforeend', getPlanningSuitePageHtml());
  injectPeriodControls();
  appMounted = true;
  configurePlanningSuiteAccess();
}
...
async function loadCurrentPeriod() {
  ...
  syncPeriodControls();
  clearDynamicRows();
  const activeUserId = window.WQStorage?.getActiveUserId?.();
  await window.WQPersonalData?.loadProfile?.(activeUserId, currentYear, currentMonth);
  await window.WQObjectives?.loadObjectives?.(activeUserId);
...
function configurePlanningSuiteAccess() {
  const show = isSuperAdmin();
  document.querySelectorAll('#tab-planning-suite, #navPlanningSuite, .planning-suite-admin-only').forEach(el => {
    if (el) el.style.display = show ? '' : 'none';
  });
  const page = R('page-planning-suite');
  if (page) page.setAttribute('aria-hidden', show ? 'false' : 'true');
}

function showPage(p) {
  configurePlanningSuiteAccess();
  if (p === 'planning-suite' && !isSuperAdmin()) {
    alert('Planning Suite is available for SuperAdmin only.');
    p = 'personal';
  }

  document.querySelectorAll('.page').forEach(e=>e.classList.remove('active'));
  document.querySelectorAll('.pill-btn').forEach(e=>e.classList.remove('active'));
  document.querySelectorAll('.nav-tab-top').forEach(e=>e.classList.remove('active'));
  R('page-'+p)?.classList.add('active');
  R('tab-'+p)?.classList.add('active');
  const navMap = { personal: 'navPersonal', entry: 'navEntry', analysis: 'navAnalytics', tracker: 'navTracker', analytics: 'navAnalytics', 'planning-suite': 'navPlanningSuite' };
  R(navMap[p])?.classList.add('active');
  if(p==='planning-suite') {
    window.WQPlanningSuite?.init?.();
  }
  if(p==='analysis') {
    renderAnalysis();
...
/* ── PRINT AS PDF ── */
function printAsPDF() {
  window.WQPDFProfile?.prepareAnalysisForPrint?.();
  document.querySelectorAll('textarea[id^="cb-"]').forEach(ta => {
    const key = ta.id.replace('cb-','');
...
function updateInvRatio() {
  ...
  if(el){ el.textContent = ratio.toFixed(2)+'%'; el.className='fh-val '+(ratio>=50?'positive':ratio>=49.9?'warning':'negative'); }
  if(tag){ tag.textContent = ratio>75 ? 'Good' : ratio>=50 ? 'Fair' : 'Below target'; tag.className='h-tag '+(ratio>=75?'tg':ratio>=50?'tw':'tb'); }
}

function renderAnalysis() {
  const data = collectData();
  const profileData = window.WQPersonalData?.getProfile?.() || {};
  const objectiveData = window.WQObjectives?.getObjectives?.() || { short: [], medium: [], long: [] };
  const hasProfileData = !!(profileData.full_name || profileData.phone || profileData.email || profileData.profession);
  const hasObjectiveData = ['short','medium','long'].some(k => (objectiveData[k] || []).length > 0);
  const hasData = hasProfileData || hasObjectiveData || data.salary>0||sumArr(data.allow)>0||sumArr(data.passive)>0||sumArr(data.side)>0;
  ...
  const fhRows=[
    {name:'Wealth Ratio (with EPF)',     fml:'Net Worth (with EPF) ÷ Expenses',              bench:'Higher is better',         val:wrWith,     fmt:v=>f2(v)+'×',  good:v=>v>=6,    warn:v=>v>=1},
    {name:'Wealth Ratio (without EPF)',  fml:'Net Worth (without EPF) ÷ Expenses',           bench:'Higher is better',         val:wrWithout,  fmt:v=>f2(v)+'×',  good:v=>v>=6,    warn:v=>v>=1},
    ...
    {name:'Saving Ratio',                fml:'(Savings + Net Cashflow) ÷ Net Income',        bench:'Benchmark ≥ 10%',           val:savRatio,   fmt:v=>f2(v)+'%',  good:v=>v>=6,    warn:v=>v>=3},
    {name:'Debt Ratio',                  fml:'Total Liabilities ÷ Total Assets',             bench:'Benchmark < 50%',            val:debtRatio,  fmt:v=>f2(v)+'%',  good:v=>v<50,    warn:v=>v<50},
    ...
    {name:'Liquid Asset to Net Worth',   fml:'Cash & Cash Equiv. ÷ Net Worth (with EPF)',   bench:'Benchmark ≥ 50%',          val:liqToNW,    fmt:v=>f2(v)+'%',  good:v=>v>=50,   warn:v=>v>=50},
    {name:'Solvency Ratio',              fml:'Net Worth (with EPF) ÷ Total Assets',          bench:'Benchmark ≥ 50%',          val:solvency,   fmt:v=>f2(v)+'%',  good:v=>v>=50,   warn:v=>v>=50},
  ];
